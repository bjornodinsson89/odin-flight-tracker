...
(function() {
    ...
    const DEFAULT_CONFIG = {
        ...
        trackFaction: true,
        trackEnemies: true,
        factionPollInterval: 30000,
        enemyPollInterval: 30000,
        manualPollInterval: 30000,
        trackedState: {},
        trackingMode: 'auto',
        manualTarget: { type: 'faction', id: null },
        alertSettings: {
            enabled: true,
    ...
    const storageCompat = (() => {
        ...
        let hasSyncGet = typeof GM_getValue === 'function';
        let hasSyncSet = typeof GM_setValue === 'function';
        let hasAsyncGet = !!(gmObj && typeof gmObj.getValue === 'function');
        let hasAsyncSet = !!(gmObj && typeof gmObj.setValue === 'function');
        ...
        function encode(value) {
            if (value === undefined) return '__ODIN_UNDEFINED__';
        ...
        function decode(raw, fallback) {
            ...
            if (raw === '__ODIN_UNDEFINED__') return fallback;
        ...
        async function getValue(key, fallback) {
            let encodedFallback = encode(fallback);
            try {
                if (hasSyncGet) {
                    let val = GM_getValue(key, encodedFallback);
            ...
            try {
                if (hasAsyncGet) {
                    let val = await gmObj.getValue(key, encodedFallback);
                    return decode(val, fallback);
                }
            } catch (_) {}
        ...
        async function setValue(key, value) {
            ...
            try {
                if (hasAsyncSet) {
                    await gmObj.setValue(key, encoded);
                    ok = true;
                }
            } catch (_) {}
    ...
    async function loadConfig() {
        CONFIG.apiKey = await storageCompat.getValue('odin_ft_apikey', DEFAULT_CONFIG.apiKey);
        CONFIG.trackFaction = await storageCompat.getValue('odin_ft_trackfaction', DEFAULT_CONFIG.trackFaction);
        CONFIG.trackEnemies = await storageCompat.getValue('odin_ft_trackenemies', DEFAULT_CONFIG.trackEnemies);
        CONFIG.factionPollInterval = await storageCompat.getValue('odin_ft_pollinterval_faction', DEFAULT_CONFIG.factionPollInterval);
        CONFIG.enemyPollInterval = await storageCompat.getValue('odin_ft_pollinterval_enemy', DEFAULT_CONFIG.enemyPollInterval);
        CONFIG.manualPollInterval = await storageCompat.getValue('odin_ft_pollinterval_manual', DEFAULT_CONFIG.manualPollInterval);
        CONFIG.trackedState = await storageCompat.getValue('odin_ft_trackedstate', DEFAULT_CONFIG.trackedState);
        CONFIG.trackingMode = await storageCompat.getValue('odin_ft_trackingmode', DEFAULT_CONFIG.trackingMode);
        CONFIG.manualTarget = await storageCompat.getValue('odin_ft_manualtarget', DEFAULT_CONFIG.manualTarget);
        CONFIG.alertSettings = await storageCompat.getValue('odin_ft_alertsettings', DEFAULT_CONFIG.alertSettings);
        CONFIG.quickViewTab = await storageCompat.getValue('odin_ft_quickviewtab', DEFAULT_CONFIG.quickViewTab);
    ...
    function normalizeLoadedConfig() {
        ...
        CONFIG.trackedState = (CONFIG.trackedState && typeof CONFIG.trackedState === 'object') ? CONFIG.trackedState : {};
        ...
        CONFIG.quickViewTab = CONFIG.quickViewTab === 'faction' ? 'faction' : 'enemies';
    }

    function saveConfig() {
        storageCompat.setValue('odin_ft_apikey', CONFIG.apiKey);
        storageCompat.setValue('odin_ft_trackfaction', CONFIG.trackFaction);
        storageCompat.setValue('odin_ft_trackenemies', CONFIG.trackEnemies);
        storageCompat.setValue('odin_ft_pollinterval_faction', CONFIG.factionPollInterval);
        storageCompat.setValue('odin_ft_pollinterval_enemy', CONFIG.enemyPollInterval);
        storageCompat.setValue('odin_ft_pollinterval_manual', CONFIG.manualPollInterval);
        storageCompat.setValue('odin_ft_trackedstate', CONFIG.trackedState);
        storageCompat.setValue('odin_ft_trackingmode', CONFIG.trackingMode);
    ...
    function updateAllUI() {
        ...
        if (ctx.faction && !ctx.war) updateFactionTitleWithId();
    ...
    function resetAllData() {
        ...
        storageCompat.setValue('odin_ft_apikey', '');
        storageCompat.setValue('odin_ft_trackfaction', true);
        storageCompat.setValue('odin_ft_trackenemies', true);
        storageCompat.setValue('odin_ft_pollinterval_faction', 30000);
        storageCompat.setValue('odin_ft_pollinterval_enemy', 30000);
        storageCompat.setValue('odin_ft_pollinterval_manual', 30000);
        storageCompat.setValue('odin_ft_trackedstate', {});
        storageCompat.setValue('odin_ft_trackingmode', 'auto');
        storageCompat.setValue('odin_ft_manualtarget', { type: 'faction', id: null });
        storageCompat.setValue('odin_ft_alertsettings', {
            enabled: true,
            tiers: { 600: true, 300: true, 60: true },
            sound: true,
            vibration: true,
    ...
    async function init() {
        if (initCompleted || initStarted) return;
        initStarted = true;
